 ...
 * @return none
 */
extern void TIM_OneShotCallbackHandler_SetCallback(TimerCallbackFunction_t *callbackFunction, uint16_t periodInSeconds);

/**
 * @brief Check if the oneshot callback slot is empty
 * @return true, if the callback slot is empty
 */
extern bool TIM_OneShotCallbackHandler_IsCallbackSlotEmpty(void);

/**
 * @brief Decrement the remaining time of the registered callback by one.
 * @return none
 */
extern void TIM_OneShotCallbackHandler_DecrementRemainingTimeByOne(void);

/**
 * @brief Fire the callback if the period is over. Delete the callback after fireing.
 * @return none
 */
extern void TIM_OneShotCallbackHandler_FireAndDeleteCallbackIfReady(void);
